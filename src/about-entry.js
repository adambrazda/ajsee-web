// src/about-entry.js
// ---------------------------------------------------------
// AJSEE – lightweight entrypoint for About page
// ---------------------------------------------------------
...
async function bootAboutEntry() {
  ...
  try {
    ...
    // About má statický header; pokud by nav-core selhal, obsah stránky zůstane funkční.
  }

  try {
    await applyTranslations(currentLang);
  } catch {
    // About texty má jako fallback i src/faq.js, takže i18n nesmí stránku blokovat.
  }
  ...
  try {
    initCookieBanner({ lang: currentLang, source: 'about-entry' });
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', bootAboutEntry, { once: true });
} else {
  bootAboutEntry();
}