// src/faq-entry.js
// ---------------------------------------------------------
// AJSEE – lightweight entrypoint for FAQ page
// ---------------------------------------------------------
...
async function bootFaqEntry() {
  ...
  try {
    ...
    // FAQ má statický header; pokud by nav-core selhal, obsah stránky zůstane funkční.
  }

  try {
    await applyTranslations(currentLang);
  } catch {
    // FAQ texty má jako fallback i src/faq.js, takže i18n nesmí stránku blokovat.
  }
  ...
  try {
    initCookieBanner({ lang: currentLang, source: 'faq-entry' });
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', bootFaqEntry, { once: true });
} else {
  bootFaqEntry();
}