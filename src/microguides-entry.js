// src/microguides-entry.js
// ---------------------------------------------------------
// AJSEE – lightweight entrypoint for microguides
// ---------------------------------------------------------
// Nahrazuje globální main.js na /microguides/.
// Cíl: ponechat základní runtime služby bez tahání events/blog/filter logiky.
...
async function bootMicroguidesEntry() {
  ...
  try {
    ...
    // Microguide obsah se renderuje z vlastního JSONu, takže i18n nesmí blokovat stránku.
  }
  ...
  try {
    initCookieBanner({ lang: currentLang, source: 'microguides-entry' });
  ...
  try {
    ...
    // Na microguides aktuálně není header/lang dropdown, takže případná absence nevadí.
  }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', bootMicroguidesEntry, { once: true });
} else {
  bootMicroguidesEntry();
}