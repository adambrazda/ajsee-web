...
export function ensureRuntimeStyles() {
  ...
  style.textContent = `
    ...
    .chips.chips-active .chip.is-active{display:inline-flex;align-items:center;gap:.5rem;cursor:pointer}
    ...
    .event-card.skeleton .ph-img{height:140px;background:linear-gradient(90deg,#eef2f6,#f6f8fb,#eef2f6);background-size:200px 100%;animation:s1 1.2s infinite}
    ...
    @media (prefers-reduced-motion: reduce){
      .event-card.skeleton .ph-img{animation:none}
    }
  `;

  const head = document.head || document.getElementsByTagName('head')[0] || document.documentElement;
  (head.appendChild ? head.appendChild(style) : document.documentElement.appendChild(style));
  injected = true;
}
